        ...
        if request.POST.get('sign_in', False):
            ...
            if user is not None:
                if user.is_active:
                    ...
                    context['sign_in_error'] = 'Пользователь не активирован'
                    context['next'] = request.POST.get('next', '/')
                    return self.render_to_response(context)
            else:
                context = self.get_context_data()
                context['next'] = request.POST.get('next', '/')
                context['sign_in_error'] = 'Не верный логин или пароль'
                return self.render_to_response(context)
        ...
        if request.POST.get('sign_up', False):
            ...
            if user is not None:
                if user.is_active:
                    ...
                    context = self.get_context_data()
                    context['sign_up_error'] = 'Пользователь не активирован'
                    context['next'] = request.POST.get('next', '/')
                    return self.render_to_response(context)
                ...
                context = self.get_context_data()
                context['next'] = request.POST.get('next', '/')
                context['sign_in_error'] = 'Что-то пошло не так'
                return self.render_to_response(context)
